...
可以看到V层和业务逻辑的部分已经被全部剥离, 剩下全是和UI相关的部分.如果觉得文件太长,可以考虑把tablesource和tabledelegate都给移植到一个单独文件以extension方式存放.

## ViewModel层

MVVM是基于MVC的扩展, 其核心目的还是在于给Controller减负.所以我们忽略胖Model等名词, 只看起点和终点, MVC中一些列逻辑放到C中就是起点, MVVM中VM把逻辑提取到VM中也就是终点.
